...
// import { CreateDocs } from '@/lib/doc';


const Sidebar = ({ docs }) => {
    const pathName = usePathname();
    const [rootNodes, setRootNodes] = useState([]);
    const [showChield, setShowChield] = useState(false);
    const [nonRootNodesGrouped, setNonRootNodesGrouped] = useState({});
    const toggleShowChield = () =>{
        setShowChield(!showChield)
    }
    useEffect(() => {
    ...
    return (
        <nav className="lg:block my-10">
            {/* <button onClick={createOne} className='bg-indigo-300 py-2 px-3 rounded-md'>Create one</button> */}
            <ul>
                <div className="relative mt-3 pl-2">
                    ...
                    <ul role="list" className="border-l border-transparent">
                        {rootNodes.map((rootNode) => (
                            <li key={rootNode.id} className="relative" onClick={toggleShowChield}>
                                <Link
                                    aria-current="page"
                                ...
                                {nonRootNodesGrouped[rootNode.id] && showChield  &&(
                                    <ul
                                        role="list"
